...
func Registered(req *register) int {
	...
	err := mysql.GlobalConn.QueryRowCtx(context.Background(), user, sql, req.Username)
	if err == nil {
		return 1
	}

	sql = "SELECT Id,UserName,Name FROM user WHERE Name = ?"
	err = mysql.GlobalConn.QueryRowCtx(context.Background(), user, sql, req.Name)
	if err == nil {
		return 2
	}
	...
	r, err := mysql.GlobalConn.ExecCtx(context.Background(), sql, newId, req.Username, req.Password, req.Name)
	if err != nil {
		return 3
	}
